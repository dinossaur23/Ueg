class VotersController < ApplicationController
  ...
  def to_json
    render json: Voter.joins(:uev, :election).where('uevs.token = ? AND elections.code = ?', params[:id], params[:election]).to_json(only: [ :name, :cpf ])
  end
  ...
  private
    ...
    def voter_params
      params.require(:voter).permit(:uev_id, :election_id, :name, :cpf, :date_of_birth, :gender)
    end
end
